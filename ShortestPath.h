...
class ShortestPath {
 ...
 public:
  ...
  int calcShortestDistancePath(const IndexVec& start, const IndexVec& goal,
                               bool onlyUseFoundWall);
  int calcShortestDistancePath(const IndexVec& start,
                               const std::list<IndexVec>& goalList,
                               bool onlyUseFoundWall);
  inline const Path& getShortestDistancePath() const {
  ...
  int calcKShortestDistancePath(const IndexVec& start, const IndexVec& goal,
                                int k, bool onlyUseFoundWall);
  int calcKShortestDistancePath(const IndexVec& start,
                                const std::list<IndexVec>& goalList, int k,
                                bool onlyUseFoundWall);
  inline const std::vector<Path>& getKShortestDistancePath() const {
    return k_shortestDistancePath;
  }
  ...
  int calcShortestTimePath(const IndexVec& start, const IndexVec& goal, int k,
                           bool onlyUseFoundWall, bool useDiagonalPath);
  int calcShortestTimePath(const IndexVec& start,
                           const std::list<IndexVec>& goalList, int k,
                           bool onlyUseFoundWall, bool useDiagonalPath);
  inline const Path& getShortestTimePath() const {
    return k_shortestDistancePath[shortestTimePath_index];
  }
  inline const OperationList& getShortestTimePathOperation() const {
    return shortestTimePath_operationList;
  }
  ...
  inline const std::list<IndexVec>& getNeedToSearchIndex() const {
    return needToSearchWallIndex;
  }
};
