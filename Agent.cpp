...
void Agent::update(const IndexVec& cur, const Direction& cur_wall) {
  ...
  if (state == Agent::SEARCHING_REACHED_GOAL) {
    ...
    if (it != distIndexList.end() || calcNextDirection(cur, dist) == 0) {
      ...
      distIndexList.clear();
      path.calcKShortestDistancePath(IndexVec(0, 0), MAZE_GOAL_LIST,
                                     SEARCH_DEPTH1, false);
      path.calcNeedToSearchWallIndex();
      distIndexList.assign(path.getNeedToSearchIndex().begin(),
                           path.getNeedToSearchIndex().end());
      if (distIndexList.empty()) {
  ...
  if (state == Agent::BACK_TO_START) {
    ...
    if (dist == cur) {
      ...
      const IndexVec diff = toDistinationPath[toDistinationPath_cnt + 1] -
                            toDistinationPath[toDistinationPath_cnt];
      for (int i = 0; i < 4; i++) {
