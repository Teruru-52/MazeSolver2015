...
struct __attribute__((__packed__)) IndexVec {
  ...
  // ベクトルとしての演算
  inline IndexVec operator+(const IndexVec& obj) const {
    return IndexVec(x + obj.x, y + obj.y);
  }
  inline IndexVec operator-(const IndexVec& obj) const {
    return IndexVec(x - obj.x, y - obj.y);
  }
  ...
  inline bool operator==(const IndexVec& obj) const {
  ...
  inline bool isCorner() {
    return x == MAZE_SIZE - 1 || x == 0 || y == MAZE_SIZE - 1 || y == 0;
  }
...
class Maze {
 ...
 public:
  ...
  void updateWall(const IndexVec& cur, const Direction& newState,
                  bool forceSetDone = true);
  ...
  // 指定座標の壁情報を取得
  inline const Direction& getWall(const IndexVec& index) const {
    return wall[index.y][index.x];
  }
  inline const Direction& getWall(int8_t x, int8_t y) const {
    return wall[y][x];
  }

  // 指定座標の歩数マップを取得
  inline const uint8_t& getStepMap(const IndexVec& index) const {
    return stepMap[index.y][index.x];
  }
  inline const uint8_t& getStepMap(int8_t x, int8_t y) const {
    return stepMap[y][x];
  }
};
