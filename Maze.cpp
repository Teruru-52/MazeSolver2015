...
const IndexVec IndexVec::vecDir[4] = {IndexVec::vecNorth, IndexVec::vecEast,
                                      IndexVec::vecSouth, IndexVec::vecWest};

void Maze::clear() {
  for (int i = 0; i < MAZE_SIZE; i++) {
    for (int j = 0; j < MAZE_SIZE; j++) { wall[i][j] = 0; }
  }
  for (int i = 0; i < MAZE_SIZE; i++) {
...
void Maze::printWall(const uint8_t value[MAZE_SIZE][MAZE_SIZE]) const {
  ...
  for (int i = 0; i < MAZE_SIZE; i++) { std::printf("-----"); }
  std::printf("+\n");
}
...
void Maze::updateWall(const IndexVec& cur, const Direction& newState,
                      bool forceSetDone) {
  // 二重書き込みを防ぐ
  ...
  for (int i = 0; i < 4; i++) {
    if (cur.canSum(IndexVec::vecDir[i])) {
      ...
      if (forceSetDone)
        ...
        wall[neighbor.y][neighbor.x] |= ((newState[i + 4] << 4) | newState[i])
                                        << (i + 2) % 4;
    }
  }
}
...
void Maze::updateStepMap(const IndexVec& dist, bool onlyUseFoundWall) {
  if (!dirty && dist == lastStepMapDist &&
      onlyUseFoundWall == lastOnlyUseFoundWall)
    return;
  ...
  for (size_t i = 0; i < MAZE_SIZE; i++) {
    for (size_t j = 0; j < MAZE_SIZE; j++) { stepMap[i][j] = 0xff; }
  }
  ...
  while (!q.empty()) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (!cur_wall[i] && stepMap[scanIndex.y][scanIndex.x] > curStep + 1) {
        ...
        // 袋小路でない場合はqueueに入れる
        if (wall[scanIndex.y][scanIndex.x].nWall() != 3) { q.push(scanIndex); }
      }
    }
  }
}
