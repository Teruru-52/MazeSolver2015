...
int ShortestPath::calcShortestDistancePath(const IndexVec& start,
                                           const IndexVec& goal,
...
int ShortestPath::calcKShortestDistancePath(const IndexVec& start,
                                            const IndexVec& goal, int _k,
                                            bool onlyUseFoundWall) {
  std::list<IndexVec> goalList;
  goalList.push_back(goal);
...
int ShortestPath::calcKShortestDistancePath(const IndexVec& start,
                                            const std::list<IndexVec>& goalList,
                                            int _k, bool onlyUseFoundWall) {
  // k=1の時は最短経路のみを計算しておわり
  if (_k == 1) {
    if (calcShortestDistancePath(start, goalList, onlyUseFoundWall) == 0)
      return 0;
  ...
  for (int k = 1; k < _k; k++) {
    for (size_t i = 0; i < k_shortestDistancePath[k - 1].size(); i++) {
      ...
      Path rootPath(k_shortestDistancePath[k - 1].begin(),
                    k_shortestDistancePath[k - 1].begin() + i + 1);
      ...
      if (calcShortestDistancePath(spurNode, spurGoal, onlyUseFoundWall) != 0) {
        ...
        rootPath.pop_back();
        std::copy(spurPath.begin(), spurPath.end(),
                  std::back_inserter(rootPath));

        // 唯一になるようにいれる
        auto rootPath_Pos_inB = std::find(B.begin(), B.end(), rootPath);
        if (rootPath_Pos_inB == B.end()) { B.push_back(rootPath); }
      }
    ...
    for (auto it = B.begin(); it != B.end();) {
      ...
      if (std::find(k_shortestDistancePath.begin(),
                    k_shortestDistancePath.end(),
                    Bitem) != k_shortestDistancePath.end()) {
        it = B.erase(it);
        continue;
      }
      it++;
...
int ShortestPath::calcShortestTimePath(const IndexVec& start,
                                       const IndexVec& goal, int k,
                                       bool onlyUseFoundWall,
                                       bool useDiagonalPath) {
  std::list<IndexVec> goalList;
  goalList.push_back(goal);
...
int ShortestPath::calcShortestTimePath(const IndexVec& start,
                                       ...
                                       bool useDiagonalPath) {
  if (calcKShortestDistancePath(start, goalList, k, onlyUseFoundWall) == 0)
    return false;
      ...
      for (int j = 0; j < 4; j++) {
        if (dxdy == IndexVec::vecDir[j]) {
          if (!maze->getWall(path[i])[j + 4]) {
            // 唯一になるようにいれる
            auto it = std::find(needToSearchWallIndex.begin(),
                                needToSearchWallIndex.end(), path[i]);
