...
float OperationList::eval() const {
  ...
  for (auto& operation : opList) {
    if (operation.op == Operation::FORWARD ||
        operation.op == Operation::FORWARD_DIAG) {
      ...
      // {
      // 	cost += (distance - 2 * accelDistance) / MAX_VELOCITY + 2 *
      // ((MAX_VELOCITY - MIN_VELOCITY) / ACCELERATION);
      // }
      // else
      // {
      // 	const float rt = std::sqrt(MIN_VELOCITY * MIN_VELOCITY + 2 *
      // ACCELERATION * distance / 2); 	cost += 2 * ((-MIN_VELOCITY + rt) /
      // ACCELERATION);
      // }
      cost += FORWARD_TIME;
    } else if (operation.op == Operation::TURN_LEFT90 ||
               operation.op == Operation::TURN_RIGHT90) {
      cost += TURN90_TIME;
    } else if (operation.op == Operation::TURN_LEFT45 ||
               operation.op == Operation::TURN_RIGHT45) {
      cost += TURN45_TIME;
    }
  }
...
void OperationList::loadFromPath(const Path& path, bool useDiagonalPath) {
  ...
  for (size_t i = 0; i < path.size() - 1; i++) {
    ...
    int8_t dir = 0;
    for (int j = 0; j < 4; j++) {
      if (dxdy == IndexVec::vecDir[j]) { dir = j; }
    }
  ...
  if (useDiagonalPath) {
    ...
    for (size_t i = 0; i < opList.size(); i++) {
      if (opList[i].op == Operation::TURN_RIGHT90 ||
          opList[i].op == Operation::TURN_LEFT90) {
        ...
        if (j - i > 1) {
          ...
          if ((j - i) % 2 == 0) {
            ...
            if (j - i - 2 > 0) {
              tmp_opList.push_back(
                  Operation(Operation::FORWARD_DIAG, j - i - 2));
            }
            if (prevDiagOp == Operation::TURN_RIGHT90)
              tmp_opList.push_back(Operation(Operation::TURN_RIGHT45));
            else
  ...
  opList.push_back(
      tmp_opList
          [1]);  // 先頭のFORWARDであるtmp_opList[0]は予め用意してあるので飛ばす
  for (size_t i = 2; i < tmp_opList.size() - 1; i++) {
    if (tmp_opList[i].op == Operation::FORWARD &&
        tmp_opList[i].op == opList.back().op)
      opList.back().n++;
    else
...
void OperationList::print() {
  for (auto operation : opList) {
    if (operation.op == Operation::START) printf("START");
    ...
    if (operation.op == Operation::STOP) printf("STOP");
