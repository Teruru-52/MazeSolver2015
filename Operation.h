...
class OperationList {
 ...
 public:
  ...
  OperationList(const Path& path, bool useDiagonalPath) {
    loadFromPath(path, useDiagonalPath);
  }
  ...
  inline std::vector<Operation>::const_iterator begin() const {
    return opList.begin();
  }
  inline std::vector<Operation>::const_iterator end() const {
    return opList.end();
  }
